...
std_estimates <- function(x) {
  # x = c(quantile(x, probs=0.025), mean(x), quantile(x, probs=0.975)) # GG changed to percentiles in res.country.rda
  # names(x) <- c('lb', 'estimate', 'ub')
  # attr(x, 'summaries') <- c('2.5%', 'mean', '97.5%')
  x = c(quantile(x, probs=0.025), quantile(x, probs=0.1), quantile(x, probs=.5), quantile(x, probs=0.9), quantile(x, probs=0.975)) # change GG 7/5/2018
  names(x) <- c('0.025', '0.1', '0.5','0.9','0.975')
  attr(x, 'summaries') <- c('0.025', '0.1', '0.5','0.9','0.975')
  return(x)
}